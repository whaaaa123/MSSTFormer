...
class DataEmbedding(nn.Module):
    def __init__(
        ...
        self.value_embedding = TokenEmbedding(1, embed_dim)
        self.value_embedding1 = TokenEmbedding(1, embed_dim)
        self.value_embedding2 = TokenEmbedding(1, embed_dim)
        ...
        self.m=nn.Linear(64,64)
        self.m1 = nn.Linear(64, 64)
        self.m2 = nn.Linear(64, 64)
    def forward(self, x, lap_mx):
        origin_x = x


        x = self.value_embedding(origin_x[:, :, :, 0].unsqueeze(-1))
...
class STSelfAttention(nn.Module):
    def __init__(
        ...
        #     nn.Linear(dim, int(dim * self.geo_ratio)) for _ in range(output_dim)
        # ])
        self.t_q_conv = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)
        self.t_k_conv = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)
        self.t_q_conv2 = nn.Conv2d(dim, 32, kernel_size=1 ,bias=qkv_bias)
        self.t_k_conv2 = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)
        self.t_v_conv = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)
        self.t_agent_conv = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)
        self.geo_q_conv = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)
        self.geo_k_conv = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)
        self.geo_v_conv = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)
        self.geo_attn_drop = nn.Dropout(attn_drop)

        self.geo_agent_conv = nn.Conv2d(dim, 32, kernel_size=1, bias=qkv_bias)

        self.sem_attn_drop = nn.Dropout(attn_drop)


        self.geo_mask=torch.from_numpy(pd.read_csv('afd.csv',header=None).values).to('cuda').bool()
        ...
        self.c = 12 // 2 + 1
        self.sqrt_beta = nn.Parameter(torch.randn(1, 1, 32))
        self.param = nn.Parameter(torch.tensor(0.5), requires_grad=True)
        self.con=nn.Conv2d(32,32,kernel_size=1, bias=qkv_bias)
    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        ...
        agent = self.geo_agent_conv(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        geo_k2 = self.geo_q_conv(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        geo_k = geo_k.reshape(B, T, N, -1, self.head_dim).permute(0, 1, 3, 2, 4)
        geo_k2 = geo_k2.reshape(B, T, N, -1, self.head_dim).permute(0, 1, 3, 2, 4)
        geo_v = geo_v.reshape(B, T, N, -1, self.head_dim).permute(0, 1, 3, 2, 4)
        agent = agent.reshape(B, T, N, -1, self.head_dim).permute(0, 1, 3, 2, 4)

        geo_attn = (agent @ geo_k.transpose(-2, -1)) * self.scale
        geo_attn2 = (agent @ geo_k2.transpose(-2, -1)) * self.scale
        geo_attn = geo_attn.softmax(dim=-1)
        geo_attn2.masked_fill_(self.geo_mask, float('-inf'))
        geo_attn2 = geo_attn2.softmax(dim=-1)
        geo_x = (geo_attn2@(geo_attn @ geo_v)).transpose(2, 3).reshape(B, T, N, int(D * self.geo_ratio))
        ...
        batch, nodes,seq_len,  features = input.shape
        ...
        low_pass=low_pass.permute(0,2,1,3).permute(0, 3, 2, 1)
        high_pass=high_pass.permute(0,2,1,3).permute(0, 3, 2, 1)
        x = x.permute(0, 3, 2, 1)
        t_q = self.t_q_conv(low_pass).permute(0, 2,3, 1)
        t_k = self.t_k_conv(low_pass).permute(0, 2,3, 1)
        t_q2 = self.t_q_conv2(high_pass).permute(0, 2,3, 1)
        t_k2 = self.t_k_conv2(high_pass).permute(0, 2,3, 1)

        t_v = self.t_v_conv(x).permute(0, 2,3, 1)
        t_q = t_q.reshape(B, N, T, 4, 8).permute(0, 1, 3, 2, 4)
        t_k = t_k.reshape(B, N, T, 4, 8).permute(0, 1, 3, 2, 4)
        t_q2 = t_q2.reshape(B, N, T, 4, 8).permute(0, 1, 3, 2, 4)
        t_k2 = t_k2.reshape(B, N, T, 4, 8).permute(0, 1, 3, 2, 4)
        t_v = t_v.reshape(B, N, T, 4, 8).permute(0, 1, 3, 2, 4)
        # t_agent=t_agent.reshape(B, N, T, 4, 8).permute(0, 1, 3, 2, 4)
        t_attn = (t_q @ t_k.transpose(-2, -1)) * (8 ** -0.5)
        t_attn2 = (t_q2 @ t_k2.transpose(-2, -1)) * (8 ** -0.5)
        t_attn = t_attn.softmax(dim=-1)
        t_attn2 = t_attn2.softmax(dim=-1)
        t_x = self.param*(t_attn @ t_v)+(1-self.param)*(t_attn2 @ t_v)
        t_x = t_x.transpose(2, 3).reshape(B, N, T, 32).permute(0,2,1,3)
        # t_x = self.con(t_x)

        x = self.proj(torch.cat([geo_x,t_x], dim=-1))
        x = self.proj_drop(x)
        return x
...
class MSSTFormer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.layers3 = nn.Conv2d(64, 64, kernel_size=(2, 1))
        self.layers2 = nn.Conv2d(1, 64, kernel_size=(3, 1))

        self.layers33 = nn.Conv2d(64, 64, kernel_size=(2, 1))
        self.layers22 = nn.Conv2d(1, 64, kernel_size=(3, 1))
